...
setwd('указать путь к рабочей директории')
...
geodata_path('указать путь к директории, в которую будут загружаться геоданные')
...
KZ_places_city = KZ_places[KZ_places$place == 'city',]
KZ_places_towm = KZ_places[KZ_places$place == 'town']
...
unique(KZ_highways$highway)
unique(KZ_highways$surface)
...
taxon = name_backbone('Crataégus')   # 3013395
taxon = name_backbone('Scabiosa')    # 2888800
taxon = name_backbone('Lonicera')  
...
# записываем в файл
write.csv(occs$data, paste0(focalTaxon,'_gbif_occurrences_.csv'), row.names = F)
...
name_backbone('Lonicera tatarica')
